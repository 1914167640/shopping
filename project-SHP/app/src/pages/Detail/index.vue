      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @blur="changeskuNum(skuNum)">
  ...
  export default {
    ...
    methods: {
      ...
      changeskuNum(skuNum) {
        // 把所有除数字以外的数据都变成1
        let regexp  = /^\d+$/
        if(!regexp.test(skuNum)) {
          this.skuNum = 1
        }
      } , 
      // 请求成功后 , 跳转到AddCartSuccess组件
      addToCart() {
        let skuId = this.$route.params.skuId
        // 向服务器发送详情属性id和购买数量
        reqAddToCart(skuId , this.skuNum)
        .then(data => {
          if(data.code === 200) {
            this.$router.push({ 
              name : 'AddCartSuccess' ,
            ...
            sessionStorage.setItem( 'skuInfo' , JSON.stringify(this.skuInfo) ) //存储skuInfo信息 , 在AddCartSuccsee组件使用
          } else {
